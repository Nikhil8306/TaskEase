...
function AddMemberTask() {
  ...
  console.log(`The userId is ${userId} and the workSpaceId is ${workSpaceId}`);
  ...
  const collaborationData = [{ label: "Individual", value: "Individual" }];

  const handleAddTask = async () => {
    try {
      const response = await fetch(
        "http://192.168.1.246:4000/workSpace/create/member/assignTask",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        navigation.navigate("MembersTask", { userId });
